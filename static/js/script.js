...
document.addEventListener('DOMContentLoaded', function () {
    ...
    loadingScreen.querySelector('.loading-spinner').after(progressBar);
    ...
    // Sample tools data - in a real app, this might come from an API
    const tools = [
        { name: 'Merge PDF', icon: 'object-group', link: 'tools/organizepdf/merge.html', category: 'Organize PDF' },
        { name: 'Split PDF', icon: 'cut', link: 'tools/organizepdf/splitpdf.html', category: 'Organize PDF' },
        { name: 'Compress PDF', icon: 'compress', link: 'tools/organizepdf/compresspdf.html', category: 'Organize PDF' },
        ...
        { name: 'PDF to PPT', icon: 'file-powerpoint', link: 'tools/convert-from-pdf/pdf-to-ppt.html', category: 'Convert from PDF' },
    ...
    // Tools Tabs with Smooth Transitions
    const tabBtns = document.querySelectorAll('.tab-btn');
    const tabPanes = document.querySelectorAll('.tab-pane');
    ...
    // Tool hover effects with animation
    document.querySelectorAll('.tool-card').forEach(card => {
        card.addEventListener('mouseenter', function () {
            this.querySelector('.tool-hover').style.transform = 'translateY(0)';
        });
    ...
    // Initialize dropdown menus with animation
    const dropdowns = document.querySelectorAll('.dropdown');
    dropdowns.forEach(dropdown => {
        dropdown.addEventListener('mouseenter', function () {
            this.querySelector('.dropdown-content').style.opacity = '1';
    ...
    try {
        if (typeof particlesJS !== 'undefined') {
            ...
            console.warn('Particles.js not loaded - background effect disabled');
        }
    } catch (e) {
    ...
    if ('loading' in HTMLImageElement.prototype) {
        ...
        // Could implement IntersectionObserver here
    }
});
